let characters = [
  ...
  let filterItems = (arr) => {
      ...
      // write a function that will filter the characters with even id numbers and return the names of those characters in uppercase	

      let filtered = arr.filter(item => item.id % 2 === 0).map(item => item.name.toUpperCase());
  ...
  // ["C-3PO", "DARTH VADER", "OWEN LARS", "HAN SOLO"]z